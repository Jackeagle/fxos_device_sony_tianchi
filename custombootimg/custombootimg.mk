...
INITSH := device/sony/tianchi/custombootimg/init.sh
BOOTREC_DEVICE := $(PRODUCT_OUT)/recovery/bootrec-device
CWM_RECOVERY := device/sony/tianchi/custombootimg/ramdisk-recovery.cpio
CWM_BUSYBOX := device/sony/tianchi/custombootimg/busybox
...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(CWM_RECOVERY) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(BOOTREC_DEVICE) $(SONY_LOGO) $(CWM_BUSYBOX) $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES) $(DTBTOOL)
